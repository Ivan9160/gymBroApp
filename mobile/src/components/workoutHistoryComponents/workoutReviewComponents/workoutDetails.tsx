...
function WorkoutDetails({}: WorkoutDetailsProps) {
    ...
    const locale = i18n.language?.toLowerCase().startsWith("uk") ? "uk-UA" : "en-US";
                ...
                <View style={styles.contentContainer}>
                    <View style={styles.historyEmpty}>
                        ...
                        <Text style={styles.historyEmptyTitle}>
                            {t("workout_details.not_found_title")}
                        </Text>

                        <Text style={styles.historyEmptyText}>
                            {t("workout_details.not_found_message")}
                        </Text>
    ...
    const currentWorkoutGroups = exerciseGroups.filter((group) =>
        workout.sets.some(
            (set: ISet) =>
                exercises.find((ex) => ex.id === set.exerciseId)?.exerciseGroupId === group.id
        )
    );
                    ...
                    <View style={styles.detailsCard}>
                        ...
                        <View style={styles.detailsHero}>
                            <View style={styles.detailsHeroContent}>
                                ...
                                <Text style={styles.detailsHeroTitle}>
                                    {t("workout_details.default_name")}
                                </Text>

                                <View style={styles.detailsDate}>
                                    <Text style={styles.detailsDateIcon}>◷</Text>
                        ...
                        <View style={[styles.detailsStats, styles.detailsStatsTwo]}>
                            <View style={styles.detailsStat}>
                                <View style={styles.detailsStatLabel}>
                                    ...
                                    <Text style={styles.detailsStatLabelText}>
                                ...
                                <Text style={styles.detailsStatValue}>
                                    {formatTime(
                                        ...
                                        new Date(workout.finishedAt || workout.createdAt)
                                    )}
                                </Text>
                            </View>

                            <View style={[styles.detailsStat, styles.detailsStatLast]}>
                                <View style={styles.detailsStatLabel}>
                                    <Text style={styles.detailsStatIcon}>●</Text>
                                    <Text style={styles.detailsStatLabelText}>
                        ...
                        <View style={styles.detailsBody}>
                            <View style={styles.detailsSectionHeading}>
                                ...
                                <Text style={styles.detailsSectionCount}>
                            ...
                            <View style={styles.detailsGroups}>
                                {currentWorkoutGroups.map(
                                    (group: IExerciseGroup, groupIndex: number) => {
                                        const currentGroupExercises = currentWorkoutExercises.filter(
                                            (exercise) => exercise.exerciseGroupId === group.id
                                        );

                                        const groupSets = workout.sets.filter((set: ISet) =>
